...
export function WebsiteBuilder() {
  ...
  return <div className="space-y-5 pb-10">
    ...
    <div className="grid gap-5 xl:grid-cols-[300px_minmax(0,1fr)_340px]">
      ...
      <Card><CardHeader><CardTitle className="text-lg">{section ? `Edit ${names[section.type]}` : 'Theme & settings'}</CardTitle></CardHeader><CardContent>{section ? <SectionEditor section={section} locale={locale} setLocale={setLocale} assets={assets} setAssets={setAssets} update={updateSection} remove={() => { changeDocument({ ...document, sections: document.sections.filter(item => item.id !== section.id) }); setSelected(null) }} /> : <Settings site={site} change={changeSite} assets={assets} setAssets={setAssets} />}</CardContent></Card>
    </div>
  </div>
}
