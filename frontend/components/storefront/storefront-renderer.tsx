...
export function StorefrontRenderer({ site, document, products, locale, preview = false }: { site: StorefrontSite; document: StorefrontDocument; products: StorefrontProduct[]; locale: Locale; preview?: boolean }) {
  ...
  return <div style={style} className="min-h-full bg-white text-slate-950">
    ...
    <main>{document.sections.filter(section => section.visible).map(section => {
      ...
      if (section.type === 'announcement') return <section key={section.id} className="px-4 py-2 text-center text-sm text-white" style={{ background: 'var(--store-primary)' }}>{body || title}</section>
      ...
      if (section.type === 'productGrid') {
        const sorted = [...products].sort((left, right) => {
          if (section.content.productSort === 'name') return left.name.localeCompare(right.name)
          if (section.content.productSort === 'priceAsc') return left.price - right.price
          if (section.content.productSort === 'priceDesc') return right.price - left.price
          return new Date(right.createdAt || 0).getTime() - new Date(left.createdAt || 0).getTime()
        })
        return <ProductGrid key={section.id} title={title} products={sorted.slice(0, section.content.productLimit || 8)} base={base} locale={locale} />
      }
