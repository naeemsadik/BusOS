...
export function WebsiteBuilder() {
  ...
  function toggleSection(id: string) { if (!document) return; changeDocument({ ...document, sections: document.sections.map(item => item.id === id ? { ...item, visible: !item.visible } : item) }) }
  function removeSection(id: string) { if (!document) return; changeDocument({ ...document, sections: document.sections.filter(item => item.id !== id) }); if (selected === id) setSelected(null) }
  function clearPage() { if (!document) return; changeDocument({ ...document, sections: [] }); setSelected(null) }
  ...
  return <div className="space-y-5 pb-10">
    ...
    <div className="grid gap-5 xl:grid-cols-[300px_minmax(0,1fr)_340px]">
      ...
      <Card><CardHeader><CardTitle className="text-lg">{section ? `Edit ${names[section.type]}` : 'Theme & settings'}</CardTitle></CardHeader><CardContent>{section ? <SectionEditor section={section} locale={locale} setLocale={setLocale} assets={assets} setAssets={setAssets} update={updateSection} remove={() => removeSection(section.id)} /> : <Settings site={site} change={changeSite} assets={assets} setAssets={setAssets} />}</CardContent></Card>
    </div>
  </div>
}
...
function SectionEditor({ section, locale, setLocale, assets, setAssets, update, remove }: { section: StorefrontSection; locale: Locale; setLocale: (value: Locale) => void; assets: StorefrontAsset[]; setAssets: (value: StorefrontAsset[]) => void; update: (change: (value: StorefrontSection) => StorefrontSection) => void; remove: () => void }) {
  const { toast } = useToast()
  const [uploading, setUploading] = useState(false)
  const content = section.content
  const field = (key: 'title' | 'body' | 'ctaLabel' | 'imageAlt' | 'hours', value: string) => update(item => ({ ...item, content: { ...item.content, [key]: { en: item.content[key]?.en || '', ...item.content[key], [locale]: value } } }))
  const supportsImage = ['hero', 'promotionalBanner', 'imageText'].includes(section.type)
  ...
  return <div className="space-y-4">
    <div className="flex gap-2"><Button size="sm" variant={locale === 'en' ? 'default' : 'outline'} onClick={() => setLocale('en')}>English</Button><Button size="sm" variant={locale === 'bn' ? 'default' : 'outline'} onClick={() => setLocale('bn')}>বাংলা</Button></div>
    <Label>Title<Input value={content.title?.[locale] || ''} onChange={event => field('title', event.target.value)} /></Label>
    <Label>Body<Textarea rows={5} value={content.body?.[locale] || ''} onChange={event => field('body', event.target.value)} /></Label>
    {locale === 'bn' && (!content.title?.bn || !content.body?.bn) && <p className="text-xs text-amber-700">Empty Bangla fields fall back to English.</p>}
    ...
    {supportsImage && <div className="space-y-3 rounded-md border p-3">
      ...
      <Label className={`flex items-center justify-center gap-2 rounded border p-3 ${uploading ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`} aria-disabled={uploading}>
        ...
        <input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" disabled={uploading} onChange={async event => { const input = event.currentTarget; await uploadImage(input.files?.[0]); input.value = '' }} />
      </Label>
      {content.imageUrl && <img src={content.imageUrl} alt={content.imageAlt?.[locale] || content.imageAlt?.en || ''} className="h-32 w-full rounded border object-cover" />}
    </div>}
    {section.type === 'categoryNavigation' && <Label>Categories, comma separated<Input value={(content.categories || []).join(', ')} onChange={event => update(item => ({ ...item, content: { ...item.content, categories: event.target.value.split(',').map(value => value.trim()).filter(Boolean) } }))} /></Label>}
    {section.type === 'productGrid' && <><p className="rounded-md bg-muted p-3 text-xs text-muted-foreground">Products are added automatically from active inventory items marked for storefront display.</p><Label>Product limit<Input type="number" min="1" max="24" value={content.productLimit || 8} onChange={event => update(item => ({ ...item, content: { ...item.content, productLimit: Number(event.target.value) } }))} /></Label><Label>Product order<Select value={content.productSort || 'newest'} onValueChange={value => update(item => ({ ...item, content: { ...item.content, productSort: value as StorefrontSection['content']['productSort'] } }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="newest">Newest first</SelectItem><SelectItem value="name">Name</SelectItem><SelectItem value="priceAsc">Price: low to high</SelectItem><SelectItem value="priceDesc">Price: high to low</SelectItem></SelectContent></Select></Label></>}
    {section.type === 'contactHours' && <Label>Opening hours<Textarea value={content.hours?.[locale] || ''} onChange={event => field('hours', event.target.value)} /></Label>}
    <Button variant="destructive" size="sm" onClick={remove}><Trash2 className="mr-2 h-4 w-4" />Remove section</Button>
  </div>
}
